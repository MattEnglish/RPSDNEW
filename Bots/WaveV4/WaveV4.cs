...
namespace Bots.WaveV4
{
        ...
        public class WaveBotV4 : IBot
        {
            public string Name => "WaveV4";
            ...
            public Weapon GetNextWeaponChoice()
            {
                ...
                if (prediction.score > highScore)
                {
                    if (memoryZone == MemoryZone.HighMemory)
                    {
                        pD = Math.Max(0, pD * (values.dynProb) * (values.dynProb));
                        pW = Math.Max(0, pW * (values.watProb) * (values.watProb));
                        pRps = Math.Max(0, pRps * (values.rpsProb) * (values.rpsProb));
                    }

                    else if (memoryZone == MemoryZone.MedMemory)
                    {
                        pD = Math.Max(0, pD * (0.05 + values.dynProb));
                        pW = Math.Max(0, pW * (0.05 + values.watProb));
                        pRps = Math.Max(0, pRps * (0.05 + values.rpsProb));
                    }
                ...
                else if (prediction.score > medScore)
                {
                    if (memoryZone == MemoryZone.HighMemory)
                    {
                        pD = Math.Max(0, pD * (0.1 + values.dynProb));
                        pW = Math.Max(0, pW * (0.1 + values.watProb));
                        pRps = Math.Max(0, pRps * (0.1 + values.rpsProb));
                    }

                    else if (memoryZone == MemoryZone.MedMemory)
                    {
                        pD = Math.Max(0, pD * (0.2 + values.dynProb));
                        pW = Math.Max(0, pW * (0.2 + values.watProb));
                        pRps = Math.Max(0, pRps * (0.2 + values.rpsProb));
                    }
